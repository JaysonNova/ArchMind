...
| 问题 | 影响 | 状态 |
|------|------|------|
| 测试覆盖率低 (~15%) | 回归测试风险高 | 待改进 |
| 缺少混合搜索重排序 | 搜索结果排序不够精准 | 待实现 |
| pgvector 2000 维度限制 | IVFFlat 索引不支持 >2000 维 | 已知限制，选模型时注意 |
| 无 Rate Limiting | API 可能被滥用 | v0.2.0 计划 |
| 无 CSRF 保护 | 跨站请求伪造风险 | v0.2.0 计划 |
| 无缓存层 | 相似查询重复计算 | v0.2.0 计划 |
