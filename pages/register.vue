      ...
      <div class="branding-content">
        ...
        <div class="feature-pills">
          <div class="pill pill-1">
            <Rocket class="pill-icon" />
            <span>{{ $t('auth.quickStart') || 'Quick Start' }}</span>
          </div>
          <div class="pill pill-2">
            <ShieldCheck class="pill-icon" />
            <span>{{ $t('auth.secure') || 'Secure' }}</span>
          </div>
          <div class="pill pill-3">
            <Cpu class="pill-icon" />
            <span>{{ $t('auth.aiPowered') || 'AI Powered' }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="auth-form-panel">
      <div class="form-wrapper">
        ...
        <form @submit="handleSubmit" class="auth-form">
          ...
          <div class="input-group" :class="{ focused: focusedField === 'fullName', filled: fullName }">
...
import { ref } from 'vue'
import {
  Sparkles, Eye, EyeOff, Loader2, AlertCircle, Mail, Lock, ArrowRight,
  User, UserPlus, Shield, Rocket, ShieldCheck, Cpu
...
const handleSubmit = async (e: Event) => {
  ...
  if (password.value !== confirmPassword.value) {
    authStore.setError(t('auth.passwordMismatch'))
    return
  }

  if (password.value.length < 8) {
    authStore.setError(t('auth.passwordMinLength'))
    return
  }
